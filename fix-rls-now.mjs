...
async function applyRLS() {
  // Use individual connection parameters to avoid URL encoding issues with # character
  const client = new Client({
    host: 'aws-1-eu-central-1.pooler.supabase.com',
    port: 6543,
    database: 'postgres',
    user: 'postgres.rzzxxkppkiasuouuglaf',
  ...
  try {
    ...
    if (error.message.includes('password authentication failed')) {
      ...
      console.log('   You need to get the database password from Supabase Dashboard:');
      console.log('   https://supabase.com/dashboard/project/rzzxxkppkiasuouuglaf/settings/database');
      console.log('\n   Then update line 22 in this script with the correct password.');
    }
