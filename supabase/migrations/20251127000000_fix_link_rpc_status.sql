-- Update link_user_to_staff_by_email to NOT auto-activate
-- Status should remain 'onboarding' until they complete profile setup
...
BEGIN
  ...
  -- Link the record BUT DO NOT CHANGE STATUS
  UPDATE staff
  SET 
    user_id = v_user_id,
    -- status = 'active',  <-- REMOVED: Keep existing status (e.g. 'onboarding')
    updated_date = now()
