...
/**
 * 🔔 NORMAL MARKETPLACE DIGEST
 *
 * Isolated parallel engine to smart-marketplace-digest.
 * Sends friendly, non-urgent notifications for normal marketplace shifts.
 */
...
async function filterEligibleShifts(supabase: any, shifts: any[], staff: any, assignedDates: string[]) {
    const now = new Date();
    return shifts.filter(shift => {
        ...
        if (shift.role_required !== staff.role) return false;

        const shiftEndDateTime = new Date(`${shift.date}T${shift.end_time}`);
        if (shift.end_time < shift.start_time) {
            shiftEndDateTime.setDate(shiftEndDateTime.getDate() + 1);
        }
        if (shiftEndDateTime < now) return false;

        if (assignedDates.includes(shift.date)) return false;
...
async function sendSMS(supabase: any, staff: any, shiftCount: number, agencyName: string, marketplaceUrl: string) {
...
Reply STOP to opt out`;
    ...
    const { error } = await supabase.functions.invoke('send-sms', {
        body: { to: normalizedPhone, message: message }
    });
    if (error) throw error;
}

async function sendWhatsApp(supabase: any, staff: any, shifts: any[], clients: any[], agencyName: string, marketplaceUrl: string) {
    const normalizedPhone = normalizePhoneNumber(staff.phone);
    let shiftList = '';
...
async function sendEmail(supabase: any, staff: any, shifts: any[], clients: any[], agencyName: string, marketplaceUrl: string) {
    let shiftCardsHtml = '';
    shifts.forEach((shift) => {
        const client = clients.find(c => c.id === shift.client_id);
        const clientName = client?.name || 'Unknown Client';
        const location = client?.address ? `${client.address.line1}, ${client.address.city} ${client.address.postcode}` : 'Location TBA';
        const contractedBreakMins = client?.contract_terms?.break_duration_minutes ?? shift.break_duration_minutes ?? 60;
        const breakApplied = (shift.duration_hours >= 10) ? contractedBreakMins : 0;
        const billableHours = Math.max(0, shift.duration_hours - (breakApplied / 60));
        const earnings = (shift.pay_rate * billableHours).toFixed(2);
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 28px;">🔔 ${shifts.length} New Shift${shifts.length > 1 ? 's' : ''} Available</h1>
        </div>
        ...
        <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
            ...
            <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
                We have <strong>${shifts.length} new shift${shifts.length > 1 ? 's' : ''}</strong> that perfectly match your profile:
            </p>
            ...
            <div style="text-align: center; margin-top: 30px;">
                <a href="${marketplaceUrl}" style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; display: inline-block;">
                    View & Claim Shifts
                </a>
            </div>

            <p style="font-size: 14px; color: #6b7280; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center;">
                ⚡ <strong>First come, first served!</strong> These shifts are visible and available to claim.
            </p>
        </div>
    ...
    const { error } = await supabase.functions.invoke('send-email', {
        body: {
            to: staff.email,
            subject: `🔔 ${shifts.length} New Shift${shifts.length > 1 ? 's' : ''} Available`,
            html: html,
...
serve(async (req) => {
    ...
    try {
        ...
        if (!shift_ids || shift_ids.length === 0 || !agency_id) {
            return new Response(
                JSON.stringify({ success: false, error: 'shift_ids[] and agency_id required' }),
                { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
            );
        }

        console.log(`🔔 [Normal Marketplace Digest] Processing ${shift_ids.length} shifts for agency ${agency_id}`);
        ...
        // ✅ Check new independent setting!
        const isEnabled = agency?.settings?.automation_settings?.use_daily_marketplace_digest === true;

        if (!isEnabled) {
            console.log('⏭️ Normal daily marketplace digest disabled for this agency');
            return new Response(
                JSON.stringify({
                    success: true,
                    skipped: true,
                    reason: 'Feature disabled in settings. Enable in Agency Settings → Automation → Daily Marketplace Digest'
                }),
        ...
        const channelSettings = agency?.settings?.urgent_shift_notifications || {
            sms_enabled: true, whatsapp_enabled: true, email_enabled: true
        };

        const branding = await getBranding(supabase, agency_id);
        const marketplaceUrl = `${branding.siteUrl}/ShiftMarketplace`;
        ...
        const results = {
            ...
            staffNotified: 0, staffSkipped: 0, totalNotificationsSent: 0,
            channelBreakdown: { sms: 0, whatsapp: 0, email: 0 },
            errors: []
        };
        ...
        for (const staff of allStaff) {
            try {
                ...
                const eligibleShifts = await filterEligibleShifts(supabase, shifts, staff, assignedShifts.map(s => s.date));
                ...
                if (channelSettings.sms_enabled && staff.phone) {
                    notificationPromises.push(sendSMS(supabase, staff, eligibleShifts.length, agency.name, marketplaceUrl)
                        .then(() => { results.channelBreakdown.sms++; })
                        .catch(err => console.error('❌ SMS failed:', err.message)));
                }

                if (channelSettings.whatsapp_enabled && staff.phone) {
                    notificationPromises.push(sendWhatsApp(supabase, staff, eligibleShifts, clients, agency.name, marketplaceUrl)
                ...
                if (channelSettings.email_enabled && staff.email) {
                    notificationPromises.push(sendEmail(supabase, staff, eligibleShifts, clients, agency.name, marketplaceUrl)
                ...
                results.errors.push({ staff_id: staff.id, error: staffError.message });
            }
        }

        console.log(`\n✅ [Normal Digest] Complete! Notified ${results.staffNotified}/${results.totalStaff} staff`);
        ...
        console.error('❌ [Normal Marketplace Digest] Fatal error:', error);
        return new Response(
