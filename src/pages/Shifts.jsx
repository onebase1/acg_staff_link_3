...
export default function Shifts() {
  ...
  // 🆕 MULTI-SELECT BROADCAST: Toggle shift selection
  ...
  // 🆕 MULTI-SELECT BROADCAST: Broadcast selected shifts using smart-marketplace-digest
  const broadcastSelectedShifts = async () => {
    ...
    // Validate all selected shifts are from same agency
    const agencyIds = [...new Set(selectedShifts.map(s => s.agency_id))];
    if (agencyIds.length > 1) {
      toast.error('Selected shifts must be from the same agency');
      return;
    }
    ...
    try {
      setIsBroadcastingSelected(true);
      console.log(`🎯 [Smart Digest] Broadcasting ${shiftIds.length} selected shifts for agency ${agencyId}`);

      // Call smart-marketplace-digest edge function
      ...
      if (data.skipped) {
        toast.warning(
          <div>
            <p className="font-semibold">Smart Digest Disabled</p>
      ...
      // Update all shifts with broadcast timestamp
      ...
      toast.success(
        <div>
          <p className="font-bold text-lg">📢 Smart Digest Sent!</p>
          <p className="text-sm mt-2">
            Notified {data.results.staffNotified} staff members about {shiftIds.length} shift{shiftIds.length > 1 ? 's' : ''}
          </p>
          <p className="text-xs mt-2">
            📊 {data.results.channelBreakdown.sms} SMS • {data.results.channelBreakdown.whatsapp} WhatsApp • {data.results.channelBreakdown.email} Email
          </p>
        </div>,
        { duration: 8000 }
      );

      console.log(`✅ [Smart Digest] Broadcast complete:`, data.results);

    } catch (error) {
      console.error('❌ [Smart Digest] Broadcast failed:', error);
      toast.error(`Broadcast failed: ${error.message}`);
    } finally {
